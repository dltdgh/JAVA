...
Article article = null/*, oldArticle = null*/;

Connection conn = DB.getConnection();
Statement stmt = DB.getStatement(conn);
ResultSet rs = DB.getRsFromId(stmt, id);
if(rs.next()){
	article = new Article();
	article.initFromRs(rs);
}

/*rs = DB.getRsFromId(conn, article.getpId());
if(rs.next()){
	oldArticle = new Article();
	oldArticle.initFromRs(rs);
}
*/
DB.close(rs);
DB.close(stmt);
